...
class Kohonen():
    ...
    def __init__(self, sizeX, sizeY):
        ...
        self.node_coords = [[x,y] for y in range(0, self.sizeY) for x in range(0,self.sizeX)]
    ...
    def get_nbr_nodes(self, bmu_idx, nbr_rad):
        ...
        euc_dis = np.sqrt(np.sum((node_coords_arr - node_coords_arr[bmu_idx])**2, axis=1))
    ...
    def update_weights_nbrs(self, t, bmu_idx, V, nbr_nodes, node_weights, time_constant, nbr_rad):
        ...
        nbr_node_coords, bmu_coords  = np.array(self.node_coords)[nbr_nodes], np.array(self.node_coords[bmu_idx])
